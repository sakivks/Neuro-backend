...
require("dotenv").config({
  path: `${__dirname}/config/environments/${process.env.NODE_ENV}.env`
});
...
app.use(async (ctx, next) => {
  try {
    ...
    console.log(
      `ERROR ::: Something went wrong, error: ${err.message}, stack: ${
        err.stack
      }`
    );
...
glob.sync("./server/**/*Route.js").forEach(function(file) {
  Object.values(require(path.resolve(file))).forEach(router =>
